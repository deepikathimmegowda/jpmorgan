...
public class TradingServiceImpl implements TradingService {
	...
	public List<TradeOutcome> computeTradeOutcome() {
		...
		for (TradeEvent event : events) {
			...
			TradeOutcome existingOutcome = outcomes.stream().filter(ele -> ele.getId().equals(id)).findFirst()
					.orElse(null);
			...
			if (existingOutcome == null) {
				...
				} else if (!existingOutcome.tradeIDContains(event.getTradeID())) {
					existingOutcome.addTradeID(event.getTradeID());
	...
	protected boolean isProductQuantityIncrement(TradeEvent event) {
		if (event.getTradeDirection() == TradeDirection.BUY
				&& (event.getTradeOperation() == TradeOperation.NEW
						|| event.getTradeOperation() == TradeOperation.AMEND)
				|| (event.getTradeDirection() == TradeDirection.SELL
						&& event.getTradeOperation() == TradeOperation.CANCEL)) {
			return true;
		}
		return false;
	}
	...
	protected boolean isProductQuantityDecrement(TradeEvent event) {
		if ((event.getTradeDirection() == TradeDirection.SELL && (event.getTradeOperation() == TradeOperation.NEW
				|| event.getTradeOperation() == TradeOperation.AMEND))
				|| (event.getTradeDirection() == TradeDirection.BUY
						&& event.getTradeOperation() == TradeOperation.CANCEL)) {
			return true;
		}
		return false;
